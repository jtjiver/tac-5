...
def generate_query_with_openai(schema_info: Dict[str, Any]) -> str:
    """
    Generate natural language query using OpenAI API based on database schema
    """
    try:
        # Get API key from environment
        api_key = os.environ.get("OPENAI_API_KEY")
...
Generate an interesting natural language query that could be asked about this data.

Requirements:
- Maximum 2 sentences
- Must reference actual table and column names from the schema
- Should be specific and insightful (not generic)
- Vary the query type: try aggregations (COUNT, AVG, SUM), filtering (WHERE), sorting (ORDER BY), limiting (LIMIT), or joins (if multiple tables available)
- Make it interesting and useful - something a user would actually want to know
- Do NOT use SQL syntax - use natural, conversational language
...
Natural Language Query:"""
        ...
        response = client.chat.completions.create(
            model="gpt-4.1-mini",
            messages=[
                {"role": "system", "content": "You are a data analyst who helps users discover interesting questions they can ask about their data."},
                {"role": "user", "content": prompt}
            ],
...
def generate_query_with_anthropic(schema_info: Dict[str, Any]) -> str:
    """
    Generate natural language query using Anthropic API based on database schema
    """
    try:
        # Get API key from environment
        api_key = os.environ.get("ANTHROPIC_API_KEY")
...
def generate_natural_language_query(schema_info: Dict[str, Any]) -> str:
    ...
    Routes to appropriate LLM provider (prioritizing OpenAI if available).
    """
    ...
    if openai_key:
        return generate_query_with_openai(schema_info)
    elif anthropic_key:
        return generate_query_with_anthropic(schema_info)
    else:
